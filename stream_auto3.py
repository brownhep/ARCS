...
bad_chans = []
#good_chans = [4,6,16,18,19,20,21,22,23,25,27]
...
for runNum in range(int(runMin),int(runMax)+1):
	for volt in reversed(Volts):
		...
		for i in regions:
			...
			preped = get_preped3(preped_name, 'PrePed', int(i), RFile_name, 1)
			#for j in xrange(512): print preped.GetBinContent(j+1)
			...
			ped = get_pedestal3(file_name, int(i), preped, 'R'+str(13-int(i)),RFile_name,scale)
			...
			#get_signal(file_name, int(i), preped, 'R'+str(13-int(i)),RFile_name,bad_chans, runNum, scale, T_Array,18,23) # *****Try running off preped***** 
			get_signal3(file_name, int(i), preped, 'R'+str(13-int(i)),RFile_name,bad_chans, runNum, scale, T_Array) # *****Try running off preped***** 
