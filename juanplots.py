...
sumfile.close()
juanfile = open ("dqdt.txt", "a")
juanfile.write("Region " + regline + "\n")
juanfile.close()
...
for r in regions:
	...
	numpts = 5
	if i > numpts-2: #Enough to do linear fit of last 5 points
		meds5 = median[i-numpts+1:i+1]
		medc5 = median_c[i-numpts+1:i+1]
		eta5 = etaary[i-numpts+1:i+1]
		tm5 = timeary[i-numpts+1:i+1]
		print tm5, meds5, medc5
		med5grph = R.TGraph(numpts)
		medc5grph = R.TGraph(numpts)
		eta5grph = R.TGraph(numpts)
		...
		dmedgraph.SetPoint(i-numpts+1, tm5[2], pmed)
		...
		dmedcgraph.SetPoint(i-numpts+1, tm5[2], pmedc)
		...
		detagraph.SetPoint(i-numpts+1, tm5[2], peta)
		...
		if (peta > petamax) and (chieta < chimaxeta):
			...
			chie = chieta
		juanfile = open("dqdt.txt","a")
		juanfile.write (str(abstime)+"\t"+str(starttemp)+"\t"+str(pmed)+"\t"+str(pmedc)+"\t"+str(peta)+"\n")
		juanfile.close()
