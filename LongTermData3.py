...
for r in regions:
	...
	temp[0] = fc[i].Get("StartTemp")[0]
        #tm[0]=(float(trial))
        MPVs.append([fc[i].Get("R"+str(region)+"_MPV0")[0],fc[i].Get("R"+str(region)+"_MPV1")[0],fc[i].Get("R"+str(region)+"_MPV2")[0]])
	...
	for i in xrange(3):
        	hist_name = 'R'+str(region)+' Signal_1000Bins_clust' + str(i)
	...
	hEta1 = fc[i].Get("Eta1")
	hEta2 = fc[i].Get("Eta2")
	etapct[0] = hEta0.Integral(30,89)/hEta.Integral()
	etapct[1] = hEta1.Integral(30,89)/hEta.Integral()
	etapct[2] = hEta2.Integral(30,89)/hEta.Integral()
