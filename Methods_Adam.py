...
def find_bad_chans(file_name, region, Ped, title, RFile_name, scale = 1, extra = None):
    ...
    if extra == None: extra = 'Region '+ str(region)
    file = open(file_name, 'r')
...
def get_signal(file_name, region, Ped, title, RFile_name, bad_chans, RunNum, scale = 1, T_Array=0, extra = None):
    ...
    if extra == None: extra = 'R'+str(region)
    file = open(file_name, 'r')	
    ...
    Raw_Data = R.TProfile("Raw Data" , "Raw Data",32,0,32)
    ...
    for line in file:
		...
		raw_event = event_data[(region+1)*32:32*(region+2)]
		
                for chan in range(len(reg_event)):
                    Sig_Data.Fill(chan, reg_event[chan])
		    Raw_Data.Fill(chan, raw_event[chan])
		...
		for i in xrange(128):
                	if abs(max_apv_chan - i) <= 2:			# Changed -- Adam Lanman 7-2-15, "- 1" to "- i" and (8-7-15) "== 1" to "<= 2"
                    		CM -= apv_event[i]
                    		count -= 1
...
#                   		CM -= apv_event[i]
#                    		count -= 1
                	if (i/4) in bad_chans: 				# Changed -- Adam Lanman 8-6-15, bad_chans refers to strips, not APV chans.
#				print "removing bad_chan", i, apv_event[i]
                    		CM -= apv_event[i]
                    		count -= 1
...
#		print CM
		   ...
		   if apv_event[max_apv_chan+1] > apv_event[max_apv_chan-1]:
			...
			if ((max_reg_chan + 1) < 32) and ((max_reg_chan - 2) > -1):
                ...
                if (max_reg_chan + 1) < 32:
                    if (apv_event[max_apv_chan+1]-CM) > 1*Ped.GetBinError(max_all_chan + 1 + 1)*R.sqrt(Ped.GetBinEntries(max_all_chan + 1 + 1)):
		...
		if any(i in neighbors for i in bad_chans) and not soft_evt[0]:
			print "Neighbor in bad chan."
			soft_evt[0] = 1
...
#			if L1[0] < 0:  L0 = 0
			...
			#if L1[0] < 0 or R1[0] < 0 or R2[0] < 0 or L2[0] < 0:
			#	print "Negative: ", L1[0], R1[0], R2[0], L2[0]
			if soft_evt[0]==0: 
				eta[0] = R0/(R0+L0)
				EtaHist.Fill(eta[0])
                	RTree.Fill()
  ...
  #  NoiCMdatDist.GetXaxis().SetTitle('Noise Distribution')
    ...
    for line in data_file:
        ...
        if line.count(',') > 3:
            ...
            full_event = [float(x) for x in line.replace('\r\t','').split('\t')[-1].split(',')]
		#Adam 6-29-15
	#    full_event = list(reversed(full_event))    # Numbering of channels is reversed from the CMS numbering.
		###
