...
for runNum in range(int(runMin),int(runMax)+1):
	for volt in reversed(Volts):
		...
		if os.path.isfile(outfolder+filePrefix+"_T"+str(runNum)): root_name = filePrefix + "_T" + str(runNum)
...
fout = open("scriptshell","w")
fout.write("#!/bin/bash \n")
fout.write(haddstr + "\n")
fout.write("python dolandau.py " + runMin + " " + runMax + " " + runinfo + "\n")
fout.write("python strip_info.py " + runMin + " " + runMax  + " " + runinfo + "\n")
fout.write("python LongTermData.py " + runMin + " " + runMax  + " " + runinfo + "\n")
fout.write("python HitDataPlots.py " + runinfo + "\n")
fout.close()
