...
def get_tempdata(file_name, outfolder, filePrefix):
	...
	if os.path.isfile(outfolder+filePrefix+"_Summary.root"): RFile = R.TFile(outfolder+filePrefix+"_Summary.root",'UPDATE')
        else:  RFile = R.TFile(outfolder+filePrefix+"_Summary.root",'RECREATE')
...
def get_preped(file_name, title, RFile_name = '', T_Data=0, scale = 1, Pedestal2 = None):
    ...
    Ped_CM = R.TH1F('PrepedCM'+title,'Preped CM',512,0,512)


    APV0_Data = R.TH2I('APV0'+title,'APV0',128,0,128,100,0,100)
    APV1_Data = R.TH2I('APV1'+title,'APV1',128,0,128,100,0,100)
    APV2_Data = R.TH2I('APV2'+title,'APV2',128,0,128,100,0,100)
    APV3_Data = R.TH2I('APV3'+title,'APV3',128,0,128,100,0,100)
    ...
    RTree = R.TTree('ppd'+title,'Ped_Data')
    ...
    for line in ped_file:
        ...
        if line[0:4] == 'Date':
            ...
            runtime = datetime(2000+int(rdate[2]),int(rdate[1]),int(rdate[0]),int(rtime[0]),int(rtime[1]),int(rtime[2])) - datetime(1900,1,1,9,0,0,0)
		...
		rmssum = 0
                for chan in range(len(event_data)): rmssum += (event_data[chan] - avgevt)*(event_data[chan] - avgevt)		
		if (rmssum/float(len(event_data))) > 100:
			NoisyEvts += 1
		else:	
		...
		Ped_CM.Fill(rmssum/float(len(event_data)))
                event_data = []
	...
	#PedStartTemp.Write("PedStartTemp")
        Ped_Data.Write()
	Ped_CM.Write()
	PedNsyEvts.Write("PedNoisyEvts"+title)
	APV0_Data.Write()
	APV1_Data.Write()
	APV2_Data.Write()
	APV3_Data.Write()
...
def get_signal(file_name, region, Ped, title, RFile_name, bad_chans, RunNum, scale = 1, T_Array=0, extra = None):
    ...
    EtaHist = R.TH1D("Eta","Eta", 100, 0.00001, 1)
    EtaHist.Sumw2()
    ...
    for line in file:
		...
		eventno[0] = event/4
                subtracted_event = [event_data[i] - Ped.GetBinContent(i+1) for i in range(512)]
                event_data = subtracted_event[(region+1)/4*128:(1+(region+1)/4)*128]
                reg_event = subtracted_event[32*(region+1):32*(region+2)]
                ...
                CM = sum(event_data) - event_data[max_apv_chan]
                count = 127.0
		...
		for i in xrange(128):
				...
				#print "removing bad_chan", i, event_data[i]
                    		CM -= event_data[i]
                    		count -= 1
		...
		if abs(CM) > 5:	
			NoisyEvt = True
                ...
                #if( event_data[max_apv_chan] > 10): print event_data[max_apv_chan], CM
                    
                #Fill unclustered histogram and tree brances
                seed_signal = event_data[max_apv_chan] - CM
		...
		if (max_reg_chan > 0) and (max_reg_chan < 31): 
		   if event_data[max_apv_chan+1] > event_data[max_apv_chan-1]:
			if ((max_reg_chan + 2) < 32) and ((max_reg_chan - 1) > -1):
				L1[0] = event_data[max_apv_chan] - CM
				R1[0] = event_data[max_apv_chan+1] - CM
				L2[0] = event_data[max_apv_chan-1] - CM
				R2[0] = event_data[max_apv_chan+2] - CM
				wholecluster = True
		   else:
			if ((max_reg_chan + 1) < 32) and ((max_reg_chan - 2) > -1):
				L1[0] = event_data[max_apv_chan-1] - CM
				R1[0] = event_data[max_apv_chan] - CM
				L2[0] = event_data[max_apv_chan-2] - CM
				R2[0] = event_data[max_apv_chan+1] - CM
				wholecluster = True

                #Do clustering and fill histograms. Change from 2 -> 1 times the noise to get counted in cluster
                if (max_reg_chan + 1) < 32:
                    total_signal += event_data[max_apv_chan+1] - CM
                    #if (event_data[max_apv_chan+1]-CM) > 1*Ped.GetBinError(max_all_chan + 1 + 1)*R.sqrt(Ped.GetBinEntries(max_all_chan + 1 + 1)): total_signal += event_data[max_apv_chan+1] - CM
                    #SR1_charge[0] = event_data[max_apv_chan+1] - CM
                    #if (max_reg_chan + 2) < 32:  SR2_charge[0] = event_data[max_apv_chan+2] - CM
                if (max_reg_chan - 1) > -1:
                    total_signal += event_data[max_apv_chan-1] - CM
                    #if (event_data[max_apv_chan-1]-CM) > 1*Ped.GetBinError(max_all_chan - 1 + 1)*R.sqrt(Ped.GetBinEntries(max_all_chan - 1 + 1)): total_signal += event_data[max_apv_chan-1] - CM
                    #SL1_charge[0] = event_data[max_apv_chan-1] - CM
                    #if (max_reg_chan - 2) > -1:  SL2_charge[0] = event_data[max_apv_chan-2] - CM
